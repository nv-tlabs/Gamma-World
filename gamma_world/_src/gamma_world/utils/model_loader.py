...
from gamma_world._src.imaginaire.flags import INTERNAL, SMOKE
...
from gamma_world._src.gamma_world.checkpointer.dcp import (
    DefaultLoadPlanner,
...
def load_model_from_checkpoint(
    experiment_name,
    s3_checkpoint_dir,
    config_file="gamma_world/_src/predict2/configs/video2world/config.py",
    enable_fsdp=False,
    ...
    text_encoder_pth: Optional[str] = None,
):
    """
    ...
    Args:
        ...
        skip_validation: skip config validation. Set to True for inference to avoid distributed broadcast issues in Ray workers.
    ...
    # Check that the config is valid (skip for inference to avoid distributed broadcast issues)
        ...
        if adapter_checkpoint_paths:
            ...
            for adapter_name, checkpoint_path in zip(adapter_names, adapter_checkpoint_paths):
                ...
                if checkpoint_path.endswith(".pt"):
                    ...
                    checkpointer = DistributedCheckpointerS3Auto(
...
def load_safetensors_to_net(net, checkpoint_path, *, strict: bool = True):
    from safetensors.torch import load_file

    def skippable(key):
        return key.endswith("_extra_state") or key.endswith("pos_embedder.seq") or key.startswith("accum_")

    loaded = load_file(checkpoint_path, device="cpu")
    ...
    missing = [k for k in target if k not in state and not skippable(k)]
    if strict and (missing or unexpected or mismatched):
        raise RuntimeError(
            f"safetensors checkpoint {checkpoint_path} does not match the network: "
...
def load_model_state_dict_from_checkpoint(
    ...
    from gamma_world._src.imaginaire.utils.checkpoint_db import get_checkpoint_path
    ...
    if checkpoint_format == "safetensors":
        if distributed.is_rank0():
            log.info(f"Loading safetensors checkpoint into model.net from {local_s3_ckpt_fp}")
            load_safetensors_to_net(model.net, local_s3_ckpt_fp, strict=True)
    ...
    if load_from_local:
        ...
        if distributed.is_rank0():
            ...
            keys_to_skip_suffix = ("pos_embedder.seq",)
            replace_keys = [k for k in local_state_dict if k.endswith(keys_to_skip_suffix)]
            model_sd = model.state_dict()
            for key in replace_keys:
                if key in local_state_dict and key in model_sd:
                    if local_state_dict[key].shape != model_sd[key].shape:
                        log.warning(
                            f"Replacing incompatible checkpoint key {key}: "
                            f"{tuple(local_state_dict[key].shape)} -> {tuple(model_sd[key].shape)}"
                        )
                        local_state_dict[key] = model_sd[key]
            model.load_state_dict(local_state_dict, strict=INTERNAL)
        ...
        checkpointer = DistributedCheckpointerS3Auto(config.checkpoint, config.job, callbacks=None, disable_async=True)
        ...
        if local_cache_dir is not None and distributed.is_rank0() and "s3://" in s3_checkpoint_dir:
            log.info(f"Caching model state dict to {local_s3_ckpt_fp}")
            os.makedirs(os.path.dirname(local_s3_ckpt_fp), exist_ok=True)
