...
from gamma_world._src.imaginaire.flags import EXPERIMENTAL_CHECKPOINTS, INTERNAL
...
def _download_hf_checkpoint(checkpoint_hf: str) -> str:
    ...
    parts = checkpoint_hf.removeprefix("hf://").split("/")
    if len(parts) < 2:
        raise ValueError(
            f"Invalid HuggingFace URI format: {checkpoint_hf}. Expected format: hf://org/repo[/path/to/file]"
        )
    repo_id = "/".join(parts[:2])
    if len(parts) == 2:
        return CheckpointDirHf(repository=repo_id, revision="main").download()
    filename = "/".join(parts[2:])
    return CheckpointFileHf(repository=repo_id, revision="main", filename=filename).download()
