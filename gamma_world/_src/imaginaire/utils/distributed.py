...
from gamma_world._src.imaginaire.utils.device import Device
...
if TYPE_CHECKING:
    from gamma_world._src.imaginaire.config import DDPConfig
...
def collate_batches(data_batches: list[dict[str, torch.Tensor]]) -> torch.Tensor | dict[str, torch.Tensor]:
    ...
    This is used for gathering validation data batches with gamma_world._src.imaginaire.utils.dataloader.DistributedEvalSampler.
