...
Usage:
    * [run all tests]: pytest -s gamma_world/_src/predict2/tests/training_loss_test.py --L1 --all 2>&1 | tee /tmp/err.log
"""
...
from gamma_world._src.imaginaire.utils.helper_test import RunIf
...
def test_training_output_patterns():
    ...
    cmd = "torchrun --nproc_per_node=1 --master_port=12341 -m scripts.train --config=gamma_world/_src/predict2/configs/text2world/config.py -- experiment=error-free_ddp_mock-data_base-cb trainer.max_iter=3 trainer.cudnn.deterministic=True trainer.cudnn.benchmark=False"
